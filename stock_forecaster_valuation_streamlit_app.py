...
def calc_gbm_params(prices: pd.Series, lookback_years: int = 3) -> Tuple[float, float]:
    """Estimate annualized drift (mu) and vol (sigma) from daily log returns."""
...
# ---------- New: simple walk-forward backtest helpers ----------
def backtest(close: pd.Series, lookback_years: int, method: str = 'GBM', horizon_days: int = 1, test_days: int = 60) -> Dict[str, float]:
    """Walk-forward 1-step or multi-step ahead backtest over the last test_days.
    Returns RMSE, MAPE, and Directional Accuracy for the chosen method.
    """
    ...
    start_idx = max(start_idx, 250)  # ensure some history
    idx = close.index
    for i in range(start_idx, len(close) - horizon_days):
        end_i = idx[i]
        # training window ends at i
        train = close.loc[:end_i]
        ...
        # ARIMA fit lazily each iter (kept small by test_days slider)
        pred_gbm = np.nan
        pred_arima = np.nan
        # GBM
        mu_i, sigma_i = calc_gbm_params(train, lookback_years=min(lookback_years, 3))
        pts_i = gbm_points(s0_i, mu_i, sigma_i, horizon_days/252)
        pred_gbm = pts_i['expected']
        # ARIMA
        try:
            log_px = np.log(train)
            best_aic = np.inf
        ...
        if method == 'GBM':
            ...
            if np.isnan(pred_arima):
                pred = pred_gbm
            else:
                pred = 0.5*pred_gbm + 0.5*pred_arima
        if np.isnan(pred):
            continue
        actual = float(close.iloc[i + horizon_days])
        actuals.append(actual)
        preds.append(pred)
        signs.append(np.sign(actual - float(close.iloc[i])))
    if not preds:
        return {"RMSE": np.nan, "MAPE%": np.nan, "Directional%": np.nan, "N": 0}
    ...
    # Directional accuracy: compare predicted change sign vs actual change sign using last train close
    # For simplicity, compare pred vs last train price
    dir_hits = 0
    for j in range(len(preds)):
        # approximate last train price as close[-horizon_days - (remaining steps)] not trivial; use benchmark: predict sign via pred > prior spot
        # In backtest loop, prior spot was s0_i; store not kept. Approx with actuals[j]/(1 + actual change sign) is messy; omit and use price comparison
        pass
    # Better: recompute sign by assuming s0_i ~ actuals[j]/(1 + realized return) not available; instead compute sign of pred delta vs actual delta from previous actual point.
    # Compute deltas from prior actual point:
    actual_prev = np.array([np.nan] + actuals[:-1].tolist())
    valid = ~np.isnan(actual_prev)
    da = np.nan
    if valid.sum() > 0:
        pred_delta = preds[valid] - actual_prev[valid]
        actual_delta = actuals[valid] - actual_prev[valid]
...
if run and ticker:
    try:
        ...
        # --------- ARIMA model (simple AIC grid search over p,q in [0..2]) ---------
        ...
        for p in range(0,3):
            for q in range(0,3):
                try:
                    model = ARIMA(log_px, order=(p,1,q))
        ...
        # --------- Chart: last 2Y and 1Y distributions ---------
        end = close.index[-1]
        ...
        sample_idx = np.linspace(0, n_paths-1, num=min(100, n_paths), dtype=int)
        ...
        if bt_enable:
            ...
            with col1:
                ...
                st.metric("GBM Directional", f"{(bt_gbm['Directional%'] if bt_gbm['Directional%'] == bt_gbm['Directional%'] else 0):.1f}%")
            with col2:
                ...
                st.metric("ARIMA Directional", f"{(bt_arima['Directional%'] if bt_arima['Directional%'] == bt_arima['Directional%'] else 0):.1f}%")
            with col3:
                ...
                st.metric("Ensemble Directional", f"{(bt_ens['Directional%'] if bt_ens['Directional%'] == bt_ens['Directional%'] else 0):.1f}%")
            st.caption("Backtest is computational and simplified; ARIMA refits each step with small (p,q) grid.")
        ...
        if comps_str.strip():
            ...
            if comp_rows:
                ...
                # format
                fmt_cols_pct = ['Profit Margin %','ROE %','Rev Growth %']
        ...
        # --------- New: Export CSV ---------
        st.subheader("Export")
        # Build a tidy DataFrame to export: forecasts + summary stats
...
# ----------------------
# Deployment Notes (create a file named requirements.txt with these lines):
# streamlit
 yfinance
 pandas
 numpy
