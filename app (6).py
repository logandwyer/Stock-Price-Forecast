...
num_cols = [c for c in df_tbl.columns if c != "Horizon"]
st.dataframe(
    df_tbl.style.format({c: "${:,.2f}" for c in num_cols}),
...
tgt = s0 * (1 + sign * pct)
prob = gbm_prob_reach(s0, mu, sigma, one_year, tgt)
st.markdown(f"**Hit probability (GBM):** {prob*100:.1f}% to reach ${tgt:,.2f} in 1Y")
    rows=[]
    for label,t in HORIZONS.items():
        rows.append({"Horizon":label,**gbm_points(s0,mu,sigma,t)})
    df_tbl = pd.DataFrame(rows)
