# -------------------------------------------------------------
# Stock Forecaster — Regimes • GARCH-like Vol • Optional ML Drift • Shocks • Diagnostics
# -------------------------------------------------------------
...
# Try Plotly; fall back to a basic line chart if unavailable
PLOTLY_OK = True
try:
...
# Session state to prevent double-runs
if "run_id" not in st.session_state:
    st.session_state.run_id = 0

# ---------------------- Data loaders (robust + cached) ----------------------
@st.cache_data(show_spinner=False, ttl=3600)
def load_price_history(ticker: str, years: int = 7) -> pd.DataFrame:
    """Robust price download with retries and sane defaults."""
    ...
    try:
        ...
        df = yf.download(ticker, period="3y", interval="1d", auto_adjust=True, progress=False, threads=False)
        if df is None or df.empty:
...
def load_macro() -> pd.DataFrame:
    """Fetch macro series individually to avoid Yahoo multi-ticker stalls."""
    def _dl(tkr, period="10y"):
        try:
            s = yf.download(tkr, period=period, interval="1d", progress=False, threads=False)["Close"]
            if s is None or s.empty:
                s = yf.download(tkr, period="5y", interval="1d", progress=False, threads=False)["Close"]
            return s.dropna()
        except Exception:
            s = yf.download(tkr, period="2y", interval="1d", progress=False, threads=False)["Close"]
            return s.dropna()

    spy = _dl("SPY")
    vix = _dl("^VIX")
    tnx = _dl("^TNX")  # 10Y yield index (tenths of pct)
    irx = _dl("^IRX")  # 13w T-bill (tenths of pct)
...
def build_ml_features(px: pd.Series, macro: pd.DataFrame) -> pd.DataFrame:
    ...
    df["target21"] = np.log(df["Close"].shift(-ML_HORIZON_DAYS)/df["Close"])  # forward 21d log return
    return df.dropna()

def fit_ml_model(feat: pd.DataFrame):
...
def predict_ml_mu(feat_latest: pd.Series, model) -> Optional[float]:
    if model is None:
        return None
    x = np.array([[
...
with st.sidebar:
    ...
    n_paths = st.slider("Paths", 500, 6000, 2500, step=500)
    seed = st.number_input("Random seed", min_value=0, value=42, step=1)

# Single-click run control
...
with tabs[0]:
    st.write("Enter a ticker and click **Run Forecast**. Chart shows the **mean** of all simulated paths.")
    if st.session_state.run_id > 0 and ticker:
        try:
            ...
            with st.spinner("Preparing model..."):
                ...
                macro_latest = {
                    "VIX": float(mlast["VIX"]),
                    "mom63": float(mlast["mom63"]) if "mom63" in macro.columns else 0.0,
                    "curve_spread": float(mlast["curve_spread"]) if "curve_spread" in macro.columns else 0.0,
                }
            ...
            with st.spinner("Running Monte Carlo simulation..."):
                # Safety clamp for UI glitches
                n_paths_safe = int(min(max(n_paths, 500), 6000))
            ...
            if PLOTLY_OK:
                ...
                st.warning("Plotly not available — showing a basic line chart instead. Add `plotly` to requirements.txt to enable interactive charts.")
                st.line_chart(pd.DataFrame({"Mean Path": mean_line}))
    ...
    if st.button("Run Diagnostics (fast)") and ticker:
        try:
            with st.spinner("Running diagnostics..."):
                ...
                macro = load_macro()
                close = prices_df["Close"]
                diag = diagnostics_walkforward(close, macro, lookback_years=lookback_years, checkpoints=10, paths_per_check=300)
            if diag.empty:
