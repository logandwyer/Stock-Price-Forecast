...
# Final, cleaned, launch-ready version with requested tweaks
# -------------------------------------------------------------
...
if run and ticker:
    try:
        ...
        # ---------- Monte Carlo (1Y) with mean line ----------
        st.subheader("Monte Carlo (GBM) – 1Y Paths with Mean")
        ...
        # Add mean line across all simulations
        mean_line = paths.mean(axis=1)
        fig2.add_trace(go.Scatter(x=np.arange(paths.shape[0]), y=mean_line, mode='lines', name='Mean of simulations', line=dict(width=3)))
        fig2.update_layout(xaxis_title="Trading Days Ahead", yaxis_title="Price")
        st.plotly_chart(fig2, use_container_width=True)

        # final prices (used for hit probability only)
